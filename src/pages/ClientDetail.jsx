    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-4 sm:p-6">
        <div className="flex items-start gap-3 sm:gap-5">
          <Link to="/clients" className="p-2 rounded-xl hover:bg-slate-100 transition-colors mt-0.5 sm:mt-1 shrink-0">
            <ArrowLeft className="w-5 h-5 text-slate-500" />
          </Link>
          <div className={`w-11 h-11 sm:w-14 sm:h-14 bg-gradient-to-br ${getAvatarColor(client.name)} rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-primary-500/15 shrink-0`}>
            <span className="text-base sm:text-lg font-bold text-white">{getInitials(client.name)}</span>
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight truncate">{client.name}</h1>
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1 sm:mt-1.5 text-xs sm:text-sm text-slate-500">
                  ...
                  <span className="hidden sm:inline w-1 h-1 rounded-full bg-slate-300" />
                  <span className="hidden sm:inline">{client.date_of_registration ? new Date(client.date_of_registration).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Not registered'}</span>
                </div>
              </div>
              {/* Desktop action buttons */}
              <div className="hidden sm:flex gap-2 shrink-0">
                <button onClick={exportAllClientData} className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-50 border border-slate-200 text-sm font-medium rounded-xl hover:bg-slate-100 transition-colors text-slate-700">
                  <Download className="w-3.5 h-3.5" /> Export
                </button>
                <Link to={`/clients/${id}/edit`} className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-50 border border-slate-200 text-sm font-medium rounded-xl hover:bg-slate-100 transition-colors text-slate-700">
                  <Pencil className="w-3.5 h-3.5" /> Edit
                </Link>
                <button onClick={deleteClient} className="inline-flex items-center gap-1.5 px-4 py-2 bg-rose-50 border border-rose-200 text-rose-600 text-sm font-medium rounded-xl hover:bg-rose-100 transition-colors">
        ...
        <div className="flex gap-2 mt-4 sm:hidden border-t border-slate-100 pt-4">
          <button onClick={exportAllClientData} className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium rounded-xl hover:bg-slate-100 transition-colors text-slate-700">
            <Download className="w-3.5 h-3.5" />
          </button>
          <Link to={`/clients/${id}/edit`} className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium rounded-xl hover:bg-slate-100 transition-colors text-slate-700 flex-1 justify-center">
            <Pencil className="w-3.5 h-3.5" /> Edit
          </Link>
          <button onClick={deleteClient} className="inline-flex items-center gap-1.5 px-3 py-2 bg-rose-50 border border-rose-200 text-rose-600 text-xs font-medium rounded-xl hover:bg-rose-100 transition-colors flex-1 justify-center">
            <Trash2 className="w-3.5 h-3.5" /> Delete
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-white rounded-xl border border-slate-200/60 p-1.5 shadow-sm overflow-x-auto scrollbar-hide">
        {tabs.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 whitespace-nowrap ${
              activeTab === tab.id
                  ...
                  <div key={inv.id} className="border border-slate-100 rounded-xl p-4 hover:border-slate-200 hover:shadow-sm transition-all bg-slate-50/30">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-10 h-10 bg-gradient-to-br ${inv.registrar === 'CAMS' ? 'from-emerald-500 to-teal-600' : 'from-sky-500 to-blue-600'} rounded-xl flex items-center justify-center shadow-sm shrink-0`}><TrendingUp className="w-4 h-4 text-white" /></div>
                        <div className="min-w-0">
                          <p className="font-semibold text-slate-900 text-sm truncate">{inv.scheme_name}</p>
                          <p className="text-xs text-slate-500 mt-0.5 truncate">{inv.fund_house} &middot; {inv.investment_type}{inv.sip_date ? ` (${inv.sip_date}th)` : ''}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-2 mt-2 ml-[52px]">
                      <p className="font-bold text-slate-900 text-sm">₹{Number(inv.amount).toLocaleString('en-IN')}</p>
